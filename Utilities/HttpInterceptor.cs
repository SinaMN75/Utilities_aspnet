...
public class CustomHttpClient<TIn, TOut> : HttpClient where TIn : class where TOut : class {
	...
	public async Task<TOut> Post(string url, TIn body, HttpRequestHeaders? headers = null) { return await SendRequest(HttpMethod.Post, url, body, headers); }

	public async Task<TOut> Get(string url, HttpRequestHeaders? headers = null) { return await SendRequest(HttpMethod.Get, url, null, headers); }

	public async Task<TOut> Put(string url, TIn body, HttpRequestHeaders? headers = null) { return await SendRequest(HttpMethod.Put, url, body, headers); }

	public async Task<TOut> Delete(string url, HttpRequestHeaders? headers = null) { return await SendRequest(HttpMethod.Get, url, null, headers); }
}
