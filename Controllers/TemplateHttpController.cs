namespace Utilities_aspnet.Controllers;

[ApiController]
[Route("api/templateHttp")]
public class TemplateHttpController : BaseApiController {
	private readonly string _token;

	public TemplateHttpController() => _token =
		"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..S-JLDDiSU8znafg226Kr6zsYMrG43dyhmFIS8x2XjiY";

	[HttpPost]
	public async Task<ActionResult<GenericResponse<AddressEntity>>> Create(AddressCreateUpdateDto dto) {
		CustomHttpClient<AddressCreateUpdateDto, GenericResponse<AddressEntity>> client = new();

		client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", _token);

		return await client.Post("https://api.sinamn75.com/api/address", dto);

		//var url = "https://api.sinamn75.com/api/address";
		//var method = HttpMethod.Post;
		//var response = await client.SendRequest(url, body, method);
	}

	[HttpPut]
	public async Task<ActionResult<GenericResponse<AddressEntity>>> Update(AddressCreateUpdateDto dto) {
		CustomHttpClient<AddressCreateUpdateDto, GenericResponse<AddressEntity>> client = new();

		client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", _token);

		return await client.Put("https://api.sinamn75.com/api/address", dto);
	}

	[HttpGet]
	public async Task<ActionResult<GenericResponse<ProductEntity>>> Get() {
		CustomHttpClient<object, GenericResponse<ProductEntity>> client = new();

		client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", _token);

		return await client.Get("https://api.sinamn75.com/api/ProductV2/14F3561A-F0EC-42B2-41C9-08DA86C505D6");
	}

	[HttpDelete]
	public async Task<ActionResult<GenericResponse>> Delete() {
		CustomHttpClient<object, GenericResponse> client = new();

		client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", _token);

		return await client.Delete("https://localhost:7125/api/address?addressId=3fa85f64-5717-4562-b3fc-2c963f66afa6");
	}
}