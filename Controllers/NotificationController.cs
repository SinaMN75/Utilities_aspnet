...
public class NotificationController : BaseApiController {
	...
	[AllowAnonymous]
	[HttpGet]
	public ActionResult<GenericResponse<IQueryable<NotificationReadDto>>> Read() => Result(_repository.Read());

	[Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
	[HttpGet("{id}")]
	public async Task<ActionResult<GenericResponse<IQueryable<NotificationReadDto>>>> ReadById(Guid id) => Result(await _repository.ReadById(id));
