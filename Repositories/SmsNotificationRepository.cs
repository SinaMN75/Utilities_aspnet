            ...
            case "faraz":
                {
                    ...
                    var body = "{\"op\" : \"pattern\"" + ",\"user\" : \"" + smsSetting.UserName + "\"" + ",\"pass\": \"" + smsSetting.SmsSecret + "\"" +
                                         ",\"fromNum\" : " + "03000505".TrimStart(new[] { '0' }) + "" + ",\"toNum\": " + mobileNumber + "" +
                                         ",\"patternCode\": \" " +
                                         smsSetting.PatternCode + "\"" + ",\"inputData\" : [{\"verification-code\":" + message + "}]}";

                    CustomHttpClient<object, string> client = new();
                    
                    using var customMessage = new HttpRequestMessage();
                    var headers = customMessage.Headers;
                    headers.Add("cache-control", "no-cache");
                    headers.Add("Authorization", "AccessKey " + smsSetting.SmsApiKey);

                    var result = await client.Post("http://ippanel.com/api/select",body, headers);
                    break;
                }
        }
    }
